...
public class SocketClient01 {
    ...
    //static boolean locker=true;         //kleidaria gia to listening. Otan einai true mporei na dex8ei aithmata ALLIWS EINAI KLEISTOS
        ...
        while (life > 0) {
            ...
            if((life >= (0.8*kwhs)) && (life <= (0.809*kwhs)) || (life >= (0.5*kwhs)) && (life <= (0.509*kwhs)) ||(life >= (0.1*kwhs)) && (life <= (0.109*kwhs))){ // Exasa to 20% tou arxikou apo8ematos || to 50% || to 90%
                ...
                if(responseMessage.equals("fail")){
                    ...
                    System.out.println("         |_[#code 69][Plan A] Description: Request Multiple Suppliers.");
                    dout.writeUTF("A");
                    dout.flush();
                   ...
                   // locker=false;////////////////////////////
                    ...
                    String the_answer  = reader2.readLine();    //STRING: NUMBER OF MULTI SUPPLIERS FOUND || "fail"
                    if(!(the_answer.equals("fail"))){           //Plan A succeed, got multi suppliers!

                        System.out.println("           [#code 69][Plan A] Message received from Server : Success.");
                        System.out.println("           [#code 69][Plan A] Number of multi-Suppliers: "+the_answer);

                        //prepei na mathw ta onomata
                        for(int j=0;j< Integer.parseInt(the_answer);j++){
                            responseMessage  = reader.readLine();
                            String current_supplier=responseMessage;
                            System.out.println("           [#code 69][Plan A] Supplier: "+current_supplier);
                            responseMessage  = reader.readLine();
                            double current_help=Double.parseDouble(responseMessage);

                            //Ready to communicate with this speciffic Supplier.
                            //..............................................
                            Socket peer_to_peer1= new Socket("169.254.245.250", port_query(current_supplier));
                            if (!peer_to_peer1.isConnected()) {
                                System.out.println("[#code 36] Socket Connection not established");
                            }else {
                                System.out.println("           [#code 69][Plan A] eisagoges_eksagoges me ton "+current_supplier);
                                double h_zwh_toy_allou=eisagoges_eksagoges(peer_to_peer1,current_help,current_supplier);
                                UpdateMyInfo(s,life,dout,username);
                                UpdateMyInfo(s,h_zwh_toy_allou,dout,current_supplier); //Update thn zwh toy allou
                                System.out.println("           [#code 69][/Plan A] Completed. ");
                        ...
                        the_answer  = reader2.readLine();    //STRING: NUMBER OF MULTI SUPPLIERS FOUND || "fail"
                        if(!(the_answer.equals("fail"))){           //Plan A succeed, got multi suppliers!

                            System.out.println("                      [#code 69][Plan B] Message received from Server : Finally Multiple Suppliers are available. Suppliers Number: " + the_answer);

                            //prepei na mathw ta onomata
                            for(int j=0;j< Integer.parseInt(the_answer);j++){
                                responseMessage  = reader.readLine();
                                String current_supplier=responseMessage;
                                System.out.println("                      [#code 69][Plan B] Supplier: " + current_supplier);
                                responseMessage  = reader.readLine();
                                double current_help=Double.parseDouble(responseMessage);
            ...
            if(life<1 && life>0){
                UpdateMyInfo(s,0,dout,username);
                System.out.println("Update Master Server about my Death.");
                dout = new DataOutputStream(s.getOutputStream());
    ...
    public void run() {
        ...
        while (true) {
            try {
                s = s2.accept();
                // CLIENT-CLIENT
                //while(SocketClient01.locker==false) {
                 //   TimeUnit.SECONDS.sleep(1);
                 //   System.out.println("Im trying to listen BUT LOCKET IS CLOSED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
                //}
